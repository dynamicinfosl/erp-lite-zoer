-- ROLLBACK: Restaurar view, policies e funções como estavam antes da migration 03f
-- Projeto: lfxietcasaooenffdodr
-- Executar caso a migration 03f cause problemas

-- ============================================================
-- 1. Recriar view SECURITY DEFINER
-- ============================================================
CREATE OR REPLACE VIEW public.cash_sessions_audit_view
...
SELECT id,
    ...
    device_info,
    status,
    (EXTRACT(epoch FROM (closed_at - opened_at)) / (3600)::numeric) AS session_hours,
        CASE
            WHEN ((security_hash IS NOT NULL) AND (is_locked = true)) THEN 'Íntegro'::text
            WHEN ((security_hash IS NULL) AND ((status)::text = 'closed'::text)) THEN 'Sem Hash'::text
            ELSE 'Aberto'::text
...
WHERE ((status)::text = 'closed'::text)
ORDER BY closed_at DESC;

-- ============================================================
-- 2. Restaurar policies antigas
-- ============================================================

-- tenants: policy INSERT permissiva
CREATE POLICY "Allow authenticated users to insert tenants" ON public.tenants
  FOR INSERT TO authenticated
  WITH CHECK (true);

-- orders: policy aberta
...
-- tabelas legacy: policies permissivas
...
-- ============================================================
-- 3. Restaurar funções sem SET search_path
-- ============================================================
...
-- ============================================================
-- 4. Restaurar permissões de EXECUTE (incluindo PUBLIC)
-- ============================================================
