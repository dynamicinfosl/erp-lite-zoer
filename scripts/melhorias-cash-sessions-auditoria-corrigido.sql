...
-- VERSÃO CORRIGIDA - Compatível com ID do tipo que está no banco
-- ============================================
...
-- Nota: Adaptada para funcionar com BIGINT ou UUID
CREATE OR REPLACE FUNCTION get_cash_session_closure_report(p_session_id TEXT)
RETURNS JSON AS $$
DECLARE
    report JSON;
BEGIN
...
-- CORRIGIDO: Usando TEXT para session_id para ser compatível com qualquer tipo
CREATE TABLE IF NOT EXISTS cash_sessions_log (
    id BIGSERIAL PRIMARY KEY,
    session_id TEXT NOT NULL, -- Usando TEXT para compatibilidade
...
-- CORRIGIDA: Usando conversão para TEXT
...
BEGIN
    IF TG_OP = 'INSERT' THEN
        INSERT INTO cash_sessions_log (
            ...
            NEW.id::TEXT, -- Converter para TEXT
            'created',
            NEW.opened_by,
            NEW.user_id,
            to_jsonb(NEW),
...
BEGIN
    ...
    RAISE NOTICE '✅ Tabela de logs criada: cash_sessions_log (compatível com BIGINT e UUID)';
